  ...
  return (
    ...
    >
      <div className="mx-auto max-w-screen-max space-y-16 md:space-y-20 lg:space-y-24 xl:space-y-28 2xl:space-y-[124px] py-16 md:py-20 lg:py-24 xl:py-28 2xl:py-[150px] px-7 md:px-10 lg:px-16 xl:px-20">
        <div
          ref={headerRef}
          className="space-y-4 md:space-y-5 lg:space-y-6 xl:space-y-7 2xl:space-y-[34px] opacity-0"
        >
          <h2 className="text-[28px] md:text-[32px] lg:text-[38px] xl:text-[44px] 2xl:text-[48px] leading-[40px] md:leading-[48px] lg:leading-[56px] xl:leading-[64px] 2xl:leading-[70px] font-extrabold">
            Why Reroute?
          </h2>
          <p className="text-[16px] md:text-[20px] lg:text-[24px] xl:text-[28px] 2xl:text-[32px] leading-[28px] md:leading-[34px] lg:leading-[40px] xl:leading-[46px] 2xl:leading-[50px] font-medium">
            리루트의 결과는 운이 아닙니다. 재현 가능한 방식과 전문성을 갖춘 팀이 함께하기
            때문입니다.
          </p>
        </div>
        ...
        <div
          ref={gridRef}
          className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-8 md:gap-10 lg:gap-12 xl:gap-14 2xl:gap-[60px] px-3 md:px-8 lg:px-12"
        >
          {cards.map((card, index) => (
            <div key={index} className="opacity-0">
              <WhyRerouteCard title={card.title} description={card.description} />
            </div>
          ))}
        </div>
      </div>
